...
## 一、命名规范
- APP 名称：`studio_django_xxx`，全小写，用下划线分隔
- Python 包与模块：全小写，短名称，避免缩写不清晰
- URL 命名空间：`app_name = 'studio_django_xxx'`
- 数据库表名：`Meta.db_table = 'xxx_model'`，避免与其他库冲突
...
## 三、接口返回规范
- 统一使用 `HertzResponse`（路径：`studio_django_utils/responses/HertzResponse.py`）
- 成功：
  ```python
...
- 视图子类化 + 路由覆盖：在项目中继承库视图并替换路由匹配
  ...
  urlpatterns = [
      ...
      path('yolo/', include('studio_django_yolo.urls', namespace='studio_django_yolo')),
  ]
  ```
- 猴子补丁：在 `AppConfig.ready()` 将库函数替换为自定义函数
  ...
  class YourAppConfig(AppConfig):
      name = 'your_app'
      def ready(self):
          from studio_django_yolo import views as yviews
...
## 九、示例：覆写 YOLO 检测返回值
- 目标位置：`studio_django_yolo/views.py:586-603`
- 最小替换示例（路由覆盖）：
  ```python
  ...
  from studio_django_utils.responses.HertzResponse import HertzResponse
  from studio_django_yolo.views import _perform_detection
  from studio_django_yolo.models import YoloModel, DetectionRecord
  ...
  def my_yolo_detection(request):
      ...
      serializer = studio_django_yolo.serializers.DetectionRequestSerializer(data=request.data)
