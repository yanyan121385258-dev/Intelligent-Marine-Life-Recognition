...
- 统一响应: 使用 `HertzResponse`，结构如下（参考 `studio_django_utils/responses/HertzResponse.py`）
  ...
  ```
- 路由挂载: 项目主路由中已通过 `path('api/notice/', include('studio_django_notice.urls'))` 挂载（`server_django/urls.py:19`）。
...
- 实现: `admin_create_notice`（`studio_django_notice/views/admin_views.py:39`）
- 请求示例:
  ```http
...
- 实现: `admin_update_notice`（`studio_django_notice/views/admin_views.py:94`）
- 请求示例:
  ```http
...
- 认证: 需要登录
- 实现: `admin_delete_notice`（`studio_django_notice/views/admin_views.py:146`）
- 请求示例:
  ```bash
...
- 实现: `admin_get_notice_list`（`studio_django_notice/views/admin_views.py:184`）
- 请求示例:
  ```bash
...
- 认证: 需要登录
- 实现: `admin_get_notice_detail`（`studio_django_notice/views/admin_views.py:273`）
- 请求示例:
  ```bash
...
- 认证: 需要登录
- 实现: `admin_publish_notice`（`studio_django_notice/views/admin_views.py:317`）
- 请求示例:
  ```bash
...
- 认证: 需要登录
- 实现: `admin_withdraw_notice`（`studio_django_notice/views/admin_views.py:374`）
- 请求示例:
  ```bash
...
- 实现: `user_get_notice_list`（`studio_django_notice/views/user_views.py:28`）
- 请求示例:
  ```bash
...
- 认证: 需要登录
- 行为: 自动标记为已读并增加查看次数
- 实现: `user_get_notice_detail`（`studio_django_notice/views/user_views.py:147`）
- 请求示例:
  ```bash
...
- 请求体: `application/json`
- 字段: `notice_id`
- 实现: `user_mark_notice_read`（`studio_django_notice/views/user_views.py:214`）
- 请求示例:
  ```http
...
- 实现: `user_batch_mark_read`（`studio_django_notice/views/user_views.py:260`）
- 请求示例:
  ```http
...
- 认证: 需要登录
- 实现: `user_mark_all_read`（`studio_django_notice/views/user_views.py:317`）
- 请求示例:
  ```bash
...
- 实现: `user_toggle_notice_star`（`studio_django_notice/views/user_views.py:401`）
- 请求示例:
  ```http
...
- 认证: 需要登录
- 实现: `user_get_notice_statistics`（`studio_django_notice/views/user_views.py:417`）
- 请求示例:
  ```bash
