...
function getEffectiveUserMenuConfigs(): UserMenuConfig[] {
  return userMenuConfigs.filter((config) => isModuleEnabled(config.moduleKey));
}

const effectiveUserMenuConfigs: UserMenuConfig[] = getEffectiveUserMenuConfigs();
...
const explicitComponentMap: Record<string, any> = {
  ...
  'DetectionHistory.vue': defineAsyncComponent(
    () => import('@/views/user_pages/DetectionHistory.vue')
  ),
