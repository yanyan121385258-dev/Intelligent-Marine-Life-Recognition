...
└─ server_diango_ui_2/              # 前端工程（Vite）
   ...
   │  ├─ stores/                          # Pinia Store
   │  │   ├─ app.ts                 # 全局应用设置（语言、布局、菜单折叠等）
   │  │   ├─ user.ts                # 用户 / 鉴权状态
   │  │   └─ theme.ts               # 主题配置与 CSS 变量
   │  ├─ styles/                          # 全局样式与变量
   │  │   ├─ index.scss                   # 全局组件风格覆盖（Button / Table / Modal ...）
   │  │   └─ variables.scss               # 主题色、阴影、圆角等变量
   │  ├─ utils/                           # 工具方法 & 基础设施
   │  │   ├─ request.ts             # Axios 封装（baseURL、拦截器、错误提示）
   │  │   ├─ url.ts                 # 统一 URL 构造（API / 媒体 / WebSocket）
   │  │   ├─ env.ts                 # 读取 & 校验 env 变量
   │  │   └─ router_utils.ts        # 路由相关工具 & 调试
   │  ├─ views/                           # 所有页面
   │  │   ├─ admin_page/                  # 管理端页面
...
- **工具函数**
  - 通用工具：`src/utils/` 下按领域拆分，如 `url.ts`、`env.ts`
...
1. **环境变量文件**（推荐只改这个）
   ...
   - `server_diango_ui_2/.env.development`
   - `server_diango_ui_2/.env.production`
...
- **扩展接口**
  ...
  - 使用统一的 `request` 封装（`request.ts`），保持错误处理一致
...
- **模块选择页面（功能 DIY）**
  ...
  - 说明：
    ...
    3. 选择结果会以 `enabled_modules` 的形式保存在浏览器 Local Storage 中。
...
- **模板模式开关**
  ...
  - 当模板模式开启且浏览器中 **没有** `enabled_modules` 记录时，路由守卫会在首次进入时自动重定向到 `/template/modules`，强制先完成模块选择。
  - 如果已经配置过模块，下次 `npm run dev` 将直接进入系统。如需重新进入模块选择页：
    1. 打开浏览器开发者工具 → Application → Local Storage
    2. 选择当前站点，删除键 `enabled_modules`
    3. 刷新页面即可再次进入模块选择流程。
